    ...
    for(size_t extruder_plan_idx = 0; extruder_plan_idx < extruder_plans.size(); extruder_plan_idx++)
    {
        ...
        if (extruder_nr != extruder_plan.extruder_nr)
        {
            ...
            if (prev_extruder_train.settings.get<bool>("retraction_hop_after_extruder_switch"))
            {
                ...
                gcode.switchExtruder(extruder_nr, storage.extruder_switch_retraction_config_per_extruder[prev_extruder], extruder_plan.paths[0].points[0], z_hop_height);
            }
            else
            {
                gcode.switchExtruder(extruder_nr, storage.extruder_switch_retraction_config_per_extruder[prev_extruder], extruder_plan.paths[0].points[0]);
            }
            ignoreZhopEnd = true;
