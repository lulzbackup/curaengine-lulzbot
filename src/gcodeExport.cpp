...
void GCodeExport::switchExtruder(size_t new_extruder, const RetractionConfig& retraction_config_old_extruder, Point first_location_of_new_extruder, coord_t perform_z_hop = 0)
{
    ...
    if(mesh_group_settings.get<bool>("prime_tower_enable"))
    {
        ...
        writeTravel(Point(x, y), Application::getInstance().current_slice->scene.extruders[current_extruder].settings.get<Velocity>("speed_travel"));
    }
    else
    {
        Settings old_extruder_settings = Application::getInstance().current_slice->scene.extruders[current_extruder].settings;
        ...
        writeTravel(target, Application::getInstance().current_slice->scene.extruders[current_extruder].settings.get<Velocity>("speed_travel"));
    }
